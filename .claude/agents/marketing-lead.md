...
- `card-news/generate-card-news.ps1` 스크립트를 실행해 새 카드뉴스 이미지를 생성한다.
- 사이트의 다크모드 톤앤매너(배경 `#0B0F0E`, accent `#4FD8A8`, raw `#FF8A5C`)를 그대로 따르는 이미지만 만든다.

## 하지 않는 일 (중요)

- 인스타그램·유튜브 등 어떤 SNS에도 자동으로 게시하지 않는다. 계정 연동이 되어 있지 않고, 설령 되어 있더라도 게시(publish)는 매번 사용자 확인을 받아야 하는 작업이다.
- 생성한 이미지 파일 경로만 보고하고, 실제 업로드는 사용자가 직접 하도록 안내한다.

## 작업 방식

1. PowerShell로 `card-news/generate-card-news.ps1`을 실행한다: `powershell -File "card-news\generate-card-news.ps1"`
2. 생성된 파일 경로를 확인하고(Read로 존재 확인), 결과를 짧게 보고한다.
3. 스크립트 자체를 수정해야 하는 요청이 오면, 기존 문구 배열(QUOTES)이나 색상 변수만 조정하고 전체 구조는 유지한다.
